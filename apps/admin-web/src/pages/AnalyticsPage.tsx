  ...
  return (
    <div className="animate-fade-in p-2">
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          <h2 className="text-3xl font-black tracking-tight" style={{ color: 'var(--foreground)' }}>Analytics</h2>
          <p className="text-sm font-medium mt-1" style={{ color: 'var(--muted-foreground)' }}>Business performance overview</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              <option value="">Download CSV Report...</option>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="card-premium p-6 relative overflow-hidden group">
              <div className="absolute -right-6 -top-6 w-32 h-32 bg-[#10b981] rounded-full opacity-[0.03] group-hover:scale-150 transition-transform duration-700 pointer-events-none" />
              <p className="text-sm font-bold uppercase tracking-wider mb-2" style={{ color: 'var(--muted-foreground)' }}>Total Revenue</p>
              <p className="text-4xl font-black" style={{ color: 'var(--foreground)' }}>₹{data.totalRevenue.toLocaleString()}</p>
            </div>
            <div className="card-premium p-6 relative overflow-hidden group">
              <div className="absolute -right-6 -top-6 w-32 h-32 bg-[#3b82f6] rounded-full opacity-[0.03] group-hover:scale-150 transition-transform duration-700 pointer-events-none" />
              <p className="text-sm font-bold uppercase tracking-wider mb-2" style={{ color: 'var(--muted-foreground)' }}>Total Orders</p>
              <p className="text-4xl font-black" style={{ color: 'var(--foreground)' }}>{data.totalOrders}</p>
            </div>
            <div className="card-premium p-6 relative overflow-hidden group">
              <div className="absolute -right-6 -top-6 w-32 h-32 bg-[#a855f7] rounded-full opacity-[0.03] group-hover:scale-150 transition-transform duration-700 pointer-events-none" />
              <p className="text-sm font-bold uppercase tracking-wider mb-2" style={{ color: 'var(--muted-foreground)' }}>Avg. Order Value</p>
              <p className="text-4xl font-black" style={{ color: 'var(--foreground)' }}>₹{data.averageOrderValue.toFixed(2)}</p>
            </div>
          </div>
          ...
          <div className="card-premium">
            <div className="px-6 py-5 border-b" style={{ borderColor: 'var(--border)' }}>
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-lg" style={{ color: 'var(--foreground)' }}>Revenue Trend</h3>
              ...
              <div className="h-[300px] w-full mt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data.revenueTrend} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                    ...
                    <XAxis dataKey="date" tick={{ fill: 'var(--muted-foreground)', fontSize: 11, fontWeight: 600 }} tickLine={false} axisLine={false} tickFormatter={(v) => v.slice(5)} />
                    <YAxis tick={{ fill: 'var(--muted-foreground)', fontSize: 11, fontWeight: 600 }} tickLine={false} axisLine={false} tickFormatter={(v) => `₹${v}`} />
                    <Tooltip
                      contentStyle={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, fontSize: 12, fontWeight: 'bold', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="card-premium">
              <div className="px-6 py-5 border-b" style={{ borderColor: 'var(--border)' }}>
                <h3 className="font-bold text-lg" style={{ color: 'var(--foreground)' }}>Most Popular Items</h3>
              </div>
              <div className="p-6">
                <div className="h-[280px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.popularItems.slice(0, 8)} layout="vertical" margin={{ top: 4, right: 16, left: 8, bottom: 0 }}>
                      ...
                      <XAxis type="number" tick={{ fill: 'var(--muted-foreground)', fontSize: 11, fontWeight: 600 }} tickLine={false} axisLine={false} />
                      <YAxis dataKey="name" type="category" width={110} tick={{ fill: 'var(--foreground)', fontSize: 11, fontWeight: 600 }} tickLine={false} axisLine={false} />
                      <Tooltip
                        contentStyle={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, fontSize: 12, fontWeight: 'bold', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }}
                        itemStyle={{ color: 'var(--foreground)' }}
            ...
            <div className="card-premium">
              <div className="px-6 py-5 border-b" style={{ borderColor: 'var(--border)' }}>
                <h3 className="font-bold text-lg" style={{ color: 'var(--foreground)' }}>Peak Hours</h3>
              </div>
              <div className="p-6">
                <div className="h-[280px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.peakHours} margin={{ top: 16 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
                      <XAxis dataKey="label" tick={{ fill: 'var(--muted-foreground)', fontSize: 10, fontWeight: 600 }} tickLine={false} axisLine={false} interval={2} />
                      <YAxis tick={{ fill: 'var(--muted-foreground)', fontSize: 11, fontWeight: 600 }} tickLine={false} axisLine={false} allowDecimals={false} />
                      <Tooltip
                        contentStyle={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, fontSize: 12, fontWeight: 'bold', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }}
                        itemStyle={{ color: 'var(--foreground)' }}
          ...
          <div className="card-premium">
            <div className="px-6 py-5 border-b" style={{ borderColor: 'var(--border)' }}>
              <h3 className="font-bold text-lg" style={{ color: 'var(--foreground)' }}>Payment Status Breakdown</h3>
            </div>
            <div className="p-6">
              {data.paymentBreakdown.length === 0 ? (
                <div className="text-sm font-semibold py-8 text-center" style={{ color: 'var(--muted-foreground)' }}>No payments in range</div>
              ) : (
                <div className="h-[280px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  <div className="p-5 rounded-2xl border" style={{ background: 'color-mix(in srgb, #3b82f6 8%, transparent)', borderColor: 'color-mix(in srgb, #3b82f6 20%, transparent)' }}>
                    <p className="text-xs font-bold uppercase tracking-wide mb-1" style={{ color: 'var(--muted-foreground)' }}>Total Discount</p>
                  ...
                  <div className="p-5 rounded-2xl border" style={{ background: 'color-mix(in srgb, #10b981 8%, transparent)', borderColor: 'color-mix(in srgb, #10b981 20%, transparent)' }}>
                    <p className="text-xs font-bold uppercase tracking-wide mb-1" style={{ color: 'var(--muted-foreground)' }}>Orders w/ Discount</p>
                  ...
                  <div className="p-5 rounded-2xl border" style={{ background: 'color-mix(in srgb, #a855f7 8%, transparent)', borderColor: 'color-mix(in srgb, #a855f7 20%, transparent)' }}>
                    <p className="text-xs font-bold uppercase tracking-wide mb-1" style={{ color: 'var(--muted-foreground)' }}>% Discounted</p>
                  ...
                  <div className="p-5 rounded-2xl border" style={{ background: 'color-mix(in srgb, #f59e0b 8%, transparent)', borderColor: 'color-mix(in srgb, #f59e0b 20%, transparent)' }}>
                    <p className="text-xs font-bold uppercase tracking-wide mb-1" style={{ color: 'var(--muted-foreground)' }}>Avg Discount/Order</p>
